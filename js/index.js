...
// end chart drawflow
  ...
  $('body').addClass('changeCursor');
	...
	$('.color-line ul li a').click(function(){
		...
		//$('.unit-icon #a').find('img').attr('src',$('.vodiapicker').find('[value='+newimgrt+']').attr('data-'+newimgrt));
	...
	$(document).ready(function(){
       ...
       $(this).attr('lastn2',n2);
		...
		$nodelast=$('.drawflow').find('.parent-node').last().find('.drawflow-node').attr('id');
		$(this).attr('lastid',$nodelast);
		//M 363.25 200 C 423.25 200 423.25 301 435  301
		//M 378.5 200 C 407    200     407 302 435.5  302
...
var currentColor = $('.color-line a.active').attr('data-color');
const settings={
	fill: 'transparent',
	background: '#d7dcdf'
  }
